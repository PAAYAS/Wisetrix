...
│   ├── json_merge.py                #   Deterministic 3-way JSON merge (large files, _diff routing)
...
│   │
│   ├── db/                          #   DB seed-data reconciliation
│   │   ├── db_scan.py               #     Derive bppol xlsx actions from app comparison
│   │   ├── xlsx_merge.py            #     Reconcile merged bizpolicydef.json into bppol xlsx
│   │   └── paths.py                 #     Locate bppol files in the DB repo
│   │
│   └── mcp/                         #   Learning store (see §19)
│       ├── lesson_store.py          #     File-backed lesson persistence + scoring
│       ├── server.py                #     /mcp REST router
│       └── hooks.py                 #     LearningMixin for BaseAgent
│
...
2. **Merge — input routing (`upgrade_api/merge_util.py`)** — before the LLM is invoked, two file classes are pulled out of the LLM inputs and handled deterministically:
   - **Large base JSON** (≥ `LARGE_JSON_BYTES`, 2 MB) → `upgrade_lib/json_merge.py` `deterministic_fill()` / `merge_json_3way()`. The LLM can't reliably hold a multi-MB JSON in context.
   - **`_diff.json` sidecars** → carried forward (large artifacts) or regenerated by `DiffAgent`. Excluded from the LLM so they don't waste turns or pressure the model into truncating the code files merged alongside them.
   The LLM merge therefore sees only the files that genuinely need semantic merging (`.java`/`.js`/`.jsp` + small JSON/XML).
3. **Quality Gate** — deterministic checks (conflict markers, JSON/XML validity, duplicate methods, import/usage) before Claude review
4. **Merge — LLM** — `MergeAgent` does the 3-way merge of the remaining files with policy context from `policies/merge_policy.md`
5. **Diff** — `DiffAgent` regenerates `_diff.json` runtime deltas per `policies/diff_format_spec.md` only when the customer artifact already contains one and the base is not a large carried-forward JSON. Bulk "Merge All" sets `skip_diff=True` to keep batch latency low; single-artifact merges default to `skip_diff=False`.
6. **DB seed-data** — for `bizpolicydefs`, `upgrade_lib/db/` derives the matching `bppol` xlsx action (Remove/Retain/Merge) and reconciles the xlsx on Merge.
7. **Review** — `ReviewAgent` produces structured PASS/WARN/FAIL with typed findings
8. **Summary** — `SummaryAgent` creates executive narrative with risk distribution + quality stats
...
## 19. The learning layer (`upgrade_lib/mcp/`)

> **Historical note:** v1 used a ~2400-line Model Context Protocol server that
> implemented JSON deep-merge, comment-aware code diff, dedup, etc. in Python —
> every bug was a semantic-misunderstanding bug. v2 deleted all of that algorithmic
> merge code; the only thing under `mcp/` today is a lightweight **learning store**,
> described below. (An earlier version of this doc claimed there was no MCP module at
> all — that was stale.)

A small feedback mechanism that lets agent runs learn from past failures:

- **`LessonStore` (`mcp/lesson_store.py`)** — thread-safe, file-backed store under
  `lessons/` (one JSON per lesson + an `index.json`). Atomic writes, keyword + category
  scoring with a threshold.
- **REST router (`mcp/server.py`)** — mounted at `/mcp` by `upgrade_api/main.py`:
  `store_lesson`, `find_lesson`, `list_lessons`, `delete`, `health`.
- **`LearningMixin` (`mcp/hooks.py`)** — `BaseAgent` inherits it. Before a Claude call
  it looks up matching lessons and injects them; after a retry-success or a final
  failure it stores a new lesson. Lookups are best-effort with a short timeout — if the
...
| | v1 | v2 |
|---|----|----|
| Algorithmic merge code | ~2400 LOC of Python | None — Claude + `merge_policy.md` |
| Agents | 1 (monolithic) | 5 (specialized) + facade |
| Source integration | Local paths only | Git + Artifactory + local |
| Large-JSON / `_diff.json` | LLM (unreliable) | Deterministic Python |
| Risk scoring | None | Deterministic per-artifact |
| Quality gates | None | Deterministic, pre-Claude |
| DB seed-data | None | `bizpolicydefs` → `bppol` xlsx reconciliation |
...
| Customer onboarding | Code changes | UI form, zero code |
